...
heatmapTitle = 'TF Enrichment in\nFootprints (-log10(X))'

# Reading Raw Data
d=as.matrix(read.table("matrix_pvalue_filtered.mtx"))
...
createHeatmap <- function(data,title,colorScheme,wid,hei,mx,my,rCex,outputFile){
  ...
  heatmap.2(data, col=colorScheme, breaks = hmbreaks, main=title, margins=c(my,mx), cexCol=0.8, cexRow=rCex, trace='none',
            sepwidth=c(2,2), sepcolor='black', Rowv=TRUE, Colv=FALSE, density.info = 'none', lhei = c(0.8, 6),
...
createHeatmapDiff <- function(data,title,colorScheme,colorBreaks,wid,hei,mx,my,rCex,outputFile){
  postscript(outputFile,width=wid,height=hei,horizontal=FALSE,paper='special')
  par(cex.axis=1.0, cex.main=0.8)
  heatmap.2(data, col=colorScheme, breaks=colorBreaks, main=title, margins=c(my,mx), cexCol=0.8, cexRow=rCex, trace='none', sepwidth=c(2,2), sepcolor='black', density.info = 'none', lhei = c(0.8, 6), dendrogram = "none", Rowv = FALSE, Colv = FALSE )
  dev.off()
  system(paste('epstopdf',outputFile,sep=' '))
  system(paste('rm',outputFile,sep=' '))
}
...
# Parameters
hmcol = colorRampPalette(brewer.pal(9, 'RdBu'))(99)
hmbreaks = c(seq(-10,10,length=100))

# Creating Heatmaps
heatmapDiffTitle = 'TF Enrichment in Footprints\n(-log10(p10)) - (-log10(p28))'
outputFileName = 'matrix_pvalue_filtered_diff.eps'
createHeatmapDiff(dFilt1_Diff,heatmapDiffTitle,hmcol,hmbreaks,wid1,hei1,mx1,my1,cexRow1,outputFileName)

# Write matrix to tab-separated txt
outputFileName = 'matrix_pvalue_filtered_diff_out.txt'
